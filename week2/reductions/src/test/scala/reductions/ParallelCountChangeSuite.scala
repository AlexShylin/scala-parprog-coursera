...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("parCountChange should work when when there is only one coin and the amount is equal to the value of the coin") {
    assert(parCountChange(1, List(1, 2), totalCoinsThreshold(2)) == 1,
      s"countChange(5, List(5)) should be 1")
  }
